// ==UserScript==
// @name         Manager Review Assistant — Loader
// @namespace    http://tampermonkey.net/
// @version      0.1.0
// @description  Loads the Manager Review Assistant script.
// @author       Oday (odemar@cisco.com)
// @match        https://scripts.cisco.com/app/quicker_csone/*
// @match        https://scripts.cisco.com/app/quicker/*
// @match        https://ss.estarta.com/ManagerReview/*
// @updateURL    https://casereview.cc/mr.user.js
// @downloadURL  https://casereview.cc/mr.user.js
// @grant        GM_setValue
// @grant        GM_getValue
...
(function () {
    ...
    console.log('[MR Loader] Loader started:', location.href);

    const API_BASE      = 'https://api.casereview.cc';
    const LOADER_SECRET = 'cra_a1b2c3d4e5f6g7h8';
    const CHECKED_KEY   = 'mrLoaderCheckedAt';
    const SHA_KEY       = 'mrLoaderCachedSha';
    const CHECK_EVERY   = 60 * 60 * 1000;

    const CACHE_KEY     = 'mrLoaderSignedCache';
    const LEGACY_CACHE  = 'mrLoaderCachedCode';
    ...
    async function verify(code, sigB64) {
        try {
            ...
            console.error('[MR Loader] Signature check errored:', e);
            return false;
        }
    }
    ...
    function runCode(code, sourceLabel) {
        try {
            ...
            console.log('[MR Loader] Script executed (' + sourceLabel + ')');
            return true;
        } catch (e) {
            console.error('[MR Loader] Execution failed (' + sourceLabel + '):', e);
            return false;
        }
    }
    ...
    async function runVerified(entry, sourceLabel) {
        if (!entry || !(await verify(entry.code, entry.sig))) {
            console.error('[MR Loader] REFUSED to run ' + sourceLabel +
    ...
    async function runFromCache(reason) {
        ...
        if (cached) {
            console.warn('[MR Loader] ' + reason + ' → running cached copy.');
            return runVerified(cached, 'cache');
        }
        console.error('[MR Loader] ' + reason + ' and no cached copy available.');
        return false;
    }
    ...
    function fetchScript(reason) {
        GM_xmlhttpRequest({
            method: 'GET',
            url: API_BASE + '/mr/script?_=' + Date.now(),
            ...
            onload: async (resp) => {
                ...
                if (!(await verify(resp.responseText, sig))) {
                    console.error('[MR Loader] Downloaded script FAILED signature verification. ' +
    ...
    async function main() {
        if (typeof GM_xmlhttpRequest !== 'function') {
            console.error('[MR Loader] GM_xmlhttpRequest unavailable.');
            return;
        }
        if (typeof crypto === 'undefined' || !crypto.subtle) {
            console.error('[MR Loader] WebCrypto unavailable — cannot verify the script, ' +
                          'so nothing was run.');
            return;
        }
        ...
        if (get(LEGACY_CACHE, null)) {
            ...
            console.log('[MR Loader] Discarded the pre-signing cache; fetching a signed copy.');
        }
        ...
        GM_xmlhttpRequest({
            method: 'GET',
            url: API_BASE + '/mr/version?_=' + Date.now(),
            headers: { 'x-cra-key': LOADER_SECRET },
            timeout: 8000,
            onload: (resp) => {
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: async (r2) => {
                        ...
                        if (!(await verify(r2.responseText, sig))) {
                            console.error('[MR Loader] Update FAILED signature verification; ' +
                                          'keeping the current copy.');
                            return;
                        }
                        writeCache(r2.responseText, sig);
                        set(SHA_KEY, meta.sha);
                        console.log('[MR Loader] Updated to ' +
